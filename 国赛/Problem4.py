...
def solve4(t, t_0):#t秒开始圆周运动，求t_0秒的位置
    ...
    time_steps = np.linspace(t_0, t_0 + 200, 1000)  # Generate 1000 time steps from 0 to 40
    positions = [get_pos(tt, 1) for tt in time_steps]

    # Extract x and y coordinates
    x_vals = [pos[0] for pos in positions]
    y_vals = [pos[1] for pos in positions]
    # Mark the center of the first arc

    # Visualize the trajectory
    plt.figure(figsize=(8, 8))
    plt.plot(x_vals, y_vals)
    plt.scatter(xo1, yo1, color="red")

    # Mark the center of the second arc
    plt.scatter(xo2, yo2, color="green")
    plt.plot([xo1, x1], [yo1, y1], color="red", linestyle="--")
    plt.plot([xo2, x3], [yo2, y3], color="green", linestyle="--")
    plt.plot([xo2, x2], [yo2, y2], color="red", linestyle="--")
    plt.plot([xo1, x3], [yo1, y3], color="green", linestyle="--")
...
for t in np.arange(x-100, x + 101):
    pos, v, flag = solve4(x, t)
    #visualization(pos, v)
    pos2 = []
